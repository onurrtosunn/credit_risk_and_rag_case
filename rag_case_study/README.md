...
- **RAG (Query-Specific)**: Analyze relevant reviews for a query (~0.8 seconds)
- **Non-RAG (Full Analysis)**: Process all 50,000 reviews (~25 minutes)
...
See [PROJECT_STRUCTURE.md](PROJECT_STRUCTURE.md) for detailed repository structure.
...
**Output**: Relevant reviews with sentiment scores (~0.8 seconds)
...
**Output**: All reviews with sentiment scores (~25 minutes)
...
Raw Data (Excel) → Data Cleaning → Cleaned Data (Parquet)
                                    ...
                                    └── FAISS (Dense)
                                         ↓
                                    Query Processing
                                    ├── RAG Pipeline (Query-Specific) → ~0.8s
                                    └── Non-RAG Pipeline (Full Dataset) → ~25min
                                         ↓
...
- **Dense Retrieval**: FAISS HNSW (Inner Product)
...
| **Speedup** | **~1500x** | RAG is significantly faster for query-specific analysis |

**Business Insight**: With RAG, we can analyze relevant reviews in under 1 second instead of scanning the entire dataset in 25 minutes.
...
- `--rrf_k`: RRF hyperparameter (default: 60)
...
# 5. Benchmark
python benchmark.py --queries "kredi" --index-dir index --input-data data/clean.parquet --output-dir outputs/benchmark

# 6. Visualization
python src/visualization.py --input outputs/full_analysis_results.parquet --output-dir outputs/charts
```

## 📚 Documentation

- [PROJECT_STRUCTURE.md](PROJECT_STRUCTURE.md): Repository structure and organization
- [CODE_REVIEW.md](CODE_REVIEW.md): Code review and improvement recommendations

## 🤝 Contributing

This is a case study project. For questions or issues, please open an issue in the repository.
...
Data Scientist Case Study - RAG Implementation

## 🔗 Links

- **Large Artifacts**: Index files and analysis results can be regenerated using the provided scripts
- **Model Downloads**: Models are automatically downloaded from Hugging Face on first use
- **Data**: Raw data file (`musteriyorumlari.xlsx`) should be provided separately
